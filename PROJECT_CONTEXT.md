...
Filter-bar con select custom (freccia SVG) + toggle "Show inactive" checkbox
Toggle "Show inactive" su tutte e 3 le pagine: Company, Warehouse, POS Client
Backend: WarehousesService.findAll() rimosso filtro isActive (come POS Client)
Frontend: showInactive signal() + filteredXxx computed() per ogni pagina
Fix ngModel su signal: pattern [ngModel]="signal()" + (ngModelChange)="signal.set($event)"
...
Salva SetupConfig in SQLite (AppConfig key="pos*setup")
