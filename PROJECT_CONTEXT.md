...
├── /users
└── /sales-report
Componenti prodotti: - ProductsComponent — lista prodotti con CRUD - ProductAddonManagerComponent — gestione addon per prodotto (standalone, Signals)
...
• ☒ ProductsService aggiornato per gestire addon inline nel create/update
• ☒ getProductsForPOS aggiornato per includere addon nella risposta sync
• ☒ Frontend: ProductAddonManagerComponent con Signals per gestire addon
• ☒ Frontend: ProductAddonService per chiamare API addon
• ☒ Frontend: Product model centralizzato in core/models/product.model.ts
• ☒ Gestione inventario, acquisti, turni, vendite
• ☒ Endpoint sync per POS (getProductsForPOS)
• ☒ Frontend admin con routing base e guard auth
• ☒ Auth JWT + PIN per utenti POS
...
• ☒ Aggiornare getProductsForPOS per includere addon nella risposta sync
• ☐ Aggiornare SaleItem / logica vendite per gestire consumo inventario addon
Fase 2: Frontend Admin Addon ✅ COMPLETATA
• ☒ Aggiornare modello Product per includere addons
• ☒ Creare componente gestione addon nella pagina prodotti
• ☒ UI per selezionare prodotti-addon dalla lista esistente
• ☒ Campo quantityValue per ogni addon item
• ☒ Campo maxQuantity a livello di ProductAddon
Fase 3: POS Client (C# WPF) 🚧 IN CORSO
• ☐ Aggiornare modello dati locale (SQLite) per ricevere addon dal sync
• ☐ UI POS: mostrare addon quando un prodotto viene selezionato
• ☐ Selezione addon con quantità (rispettando maxQuantity)
• ☐ Inviare addon nella struttura SaleItem al backend
...
Frontend Admin https://github.com/LorisLancia/POS-Enterprise-Frontend-Angular.git Angular 21 (Signals)
...
• ☒ Frontend admin con routing base e guard auth
• ☒ Auth JWT + PIN per utenti POS
• ☒ POS Client: LocalProductAddon, LocalProductAddonItem, LocalSaleItemAddon modelli C#
• ☒ POS Client: SyncService aggiornato per scaricare addon dal backend
• ☒ POS Client: POSDbContext aggiornato con DbSet per addon
...
• ☒ Aggiornare getProductsForPOS per includere addon nella risposta sync
• ☐ Aggiornare SaleItem / logica vendite per gestire consumo inventario addon
Fase 2: Frontend Admin Addon ✅ COMPLETATA (con limitazioni)
• ☒ Aggiornare modello Product per includere addons
• ☒ Creare componente gestione addon nella pagina prodotti
• ☒ UI per selezionare prodotti-addon dalla lista esistente
• ☒ Campo quantityValue per ogni addon item
• ☒ Campo maxQuantity a livello di ProductAddon
• ☐ Manca: Pagina /categories per gestire ProductCategory
• ☐ Manca: Form prodotto completo con dropdown categoria, varianti, ricette, modifier
• ☐ Manca: Allineamento modello frontend con backend (campi price vs basePrice, category vs categoryId)
Fase 3: POS Client (C# WPF) 🚧 IN CORSO
...
• ☐ Manca: UI WPF per mostrare addon quando un prodotto viene selezionato
• ☐ Manca: Selezione addon con quantità (rispettando maxQuantity)
• ☐ Manca: Inviare addon nella struttura SaleItem al backend
• ☐ Manca: Gestire offline sync per addon
Fase 4: Inventario & Reporting
• ☐ Consumo materiale per addon nelle vendite
• ☐ Report margini che includono costo addon
• ☐ Alert stock per materiali consumati come addon
...
1. Frontend: Pagina Categorie (/categories)
   • CRUD semplice: nome, colore, sortOrder
   • API backend: GET /product-categories, POST, PATCH, DELETE
   • Routing: aggiungere /categories in app.routes.ts
2. Frontend: Aggiornare form Prodotto
   • Dropdown Categoria (da API /product-categories)
   • Sezione Ricetta: seleziona materiale + quantità + unità
   • Sezione Varianti: nome + priceAdjustment
   • Sezione Modifier: seleziona da lista esistente
   • Sezione Addon: già fatto (ProductAddonManager)
3. Frontend: Allineamento modello
   • Sostituire price con basePrice
   • Sostituire category: string con categoryId: number
   • Aggiornare ProductsService per usare i campi corretti del backend
4. POS Client: UI Addon
   • Mostrare addon quando un prodotto viene selezionato nella vendita
   • Selezione con quantità e rispetto maxQuantity
   • Salvare addon nella vendita locale
