...
Product Refactor — Ricette nestate, Addon price, Edit completo (2026-06-27)

- Backend DTO: ProductVariantDto con recipes[] interno (approccio B)
- Backend Service: createProduct() e update() con ricette nestate per variante
- Backend: Modulo ModifierGroups dedicato (/modifier-groups endpoint)
- Frontend: ProductComponent con form varianti + ricette nestate
- Frontend: Addon form con price custom per ogni item
- Frontend: Edit prodotto completo (popola varianti, ricette, modifier, addon)
- Fix: price su ProductAddonItemDto (create + update DTO)
- Fix: barcode/description rimossi da ProductUpdateInput (non esistono in schema)
  ...
  Cosa manca / Roadmap 🚧
  Fase 1: ☐ Migration refactor_remove_store (se non ancora fatto)
  ...
  ☐ Loading skeletons
  ✅ Form prodotto edit con varianti/ricette/modifier esistenti — COMPLETATO
